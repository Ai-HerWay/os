...
## 9. Inputs and memory
Reads: the incoming request, the live calendar/connection, `memory/business-context.md` (working hours, focus blocks, VIPs, meeting-length defaults, consecutive-meeting cap, routing rules, voice, transparency line), and `memory/industry-context.md` if present (sector norms on call etiquette); `admin-ops-os/memory/admin-settings.md` (this department's own settings: triage stance, VIPs, urgent and escalation channels, calendar buffer, SOP threshold, travel preferences). Writes: `logs/activity-log.md` (requests handled, format chosen, time protected) and `logs/decision-log.md` (any judgement call, VIP override, or challenged recurring meeting).
