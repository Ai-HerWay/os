...
## 9. Inputs and memory
Reads: the wiki and process or SOP files in scope, `memory/business-context.md` (review window, canonical locations, page owners, voice, transparency note, escalation channel), `memory/industry-context.md` if present (sector-specific documentation and retention rules), `logs/decision-log.md` (recent decisions that may outdate pages), `logs/activity-log.md` (what was last reviewed); `admin-ops-os/memory/admin-settings.md` (this department's own settings: triage stance, VIPs, urgent and escalation channels, calendar buffer, SOP threshold, travel preferences). Writes: drafted corrections and new pages to the working location named in `business-context.md` (marked `[DRAFT]` until approved), `logs/activity-log.md` (pages reviewed, states assigned, next-review dates), `logs/decision-log.md` (unresolved contradictions and consequential calls flagged for review).
