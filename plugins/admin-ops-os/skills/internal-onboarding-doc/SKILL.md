...
## 9. Inputs and memory
Reads: `memory/business-context.md` (role name, people, tools, voice, boundaries, transparency line), `memory/industry-context.md` if present (sector norms and never-do considerations), the existing process sources for the role (SOPs, prior handover notes), and `logs/activity-log.md` for recent history; `admin-ops-os/memory/admin-settings.md` (this department's own settings: triage stance, VIPs, urgent and escalation channels, calendar buffer, SOP threshold, travel preferences). Writes: `logs/activity-log.md` (what was produced and from which sources), `logs/decision-log.md` (any judgement call, conflict, or gap flagged), and the named output document (default `onboarding-[role-name].md`).
