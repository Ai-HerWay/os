...
4. Add or update the client's row in `memory/client-roster.md` (the CRM-lite mirror of the member's client database, where a Won pipeline row becomes a roster row): client, engagement, stage set to Kickoff, start date, first milestone, cadence, renewal or end date, and the brief's file path.
